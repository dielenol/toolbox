...
Toolbox는 로컬 전용이므로 실행 호스트는 `127.0.0.1`, `localhost`, `::1`만 허용합니다.
...
| 모델 | 용도 |
| --- | --- |
| `birefnet-hq` | 범용 최고. 사진, 제품, 머리카락처럼 용도가 섞였을 때 먼저 선택 |
| `birefnet-massive` | 캐릭터 최고. 캐릭터, 피규어, 복잡한 실루엣용 |
| `birefnet-hrsod` | 로고/제품 최고. 선명한 제품, 로고, 물체 윤곽용 |
| `birefnet-portrait` | 인물 최고. 프로필, 상반신, 사람 중심 이미지용 |
| `isnet-anime` | 애니 최고. 일러스트와 2D 캐릭터용 |
| `birefnet-general-lite` | 벌크 최고. 여러 장을 처리할 때 품질과 속도 균형용 |

### 프리셋

| 프리셋 | 설명 |
| --- | --- |
| `Ultra` | 선택한 목적별 모델은 유지하고 가장 정밀한 보정 옵션을 씁니다. |
| `Studio` | 선택한 모델은 유지하고 품질 우선 보정 옵션을 씁니다. |
| `Balanced` | 선택한 모델은 유지하고 속도와 품질의 중간 보정 옵션을 씁니다. |
| `Fast` | 선택한 모델은 유지하고 빠른 확인용 보정 옵션을 씁니다. |
